...
export const RatingCard = ({ rating, variant = "defaut" }: RatingCardProps) => {
    ...
    <Container variant={variant}>
        ...
        <BookDetails>
          <Link href={`/explore?$book=${rating.book_id}`}>
            <BookImage width={108} height={152} alt={rating.book_name} src={rating.book.cover_url} />
          </Link>
          ...
          <BookContent>
            <div>
              ...
              <Text size="sm" color="gray400">{rating.book.author}</Text>
            </div>

            <Text size="sm" color="gray300" css={{marginTop: '$5'}}>
              {bookSummary}
